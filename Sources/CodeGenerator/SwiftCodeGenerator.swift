...
struct SwiftCodeGenerator {
    ...
    private static let preamble = [
        ...
        "import AEXML",
        "import Foundation",
        "typealias XMLDeserializable = XMLDeserialize & XMLSerialize",
        ""].joined(separator: "\n")
}
...
extension SwiftTypeClass {
    ...
    private func deserializer(at indentation: Indentation) -> [LineOfCode] {
        ...
        return indentation.apply(
            toFirstLine: "required init(deserialize element: AEXMLElement) throws {",
            nestedLines:
                properties.flatMap { property -> [LineOfCode] in
                    let element = property.element.name
                    let throwNoElement = "throw NewXMLDeserializationError.noElementWithName(QualifiedName(uri: \"\(element.uri)\", localName: \"\(element.localName)\"))"
                    switch property.type {
                    case let .identifier(identifier):
                        return [
                            "do {",
                            "    guard let node = element.firstDescendantSingleGeneration(where: {$0.name == \"\(element.localName)\"})  else {",
                            "        \(throwNoElement)",
                            "    }",
                    ...
                    case let .optional(.identifier(identifier)):
                        return [
                            "do {",
                            "    if let node = element.firstDescendantSingleGeneration(where: {$0.name == \"\(element.localName)\"}) {",
                            "        self.\(property.name) = try \(identifier)(deserialize: node)",
                            "    } else {",
...
//                            "    }",
                    ...
                    case let .array(.identifier(identifier)):
                        return [
                            "self.\(property.name) = try  element.allDescendants(where: {$0.name == \"\(element.localName)\"}).map(\(identifier).init(deserialize:))"
                        ]
                    case let .array(.nillable(.identifier(identifier))):
                        return [
                            "self.\(property.name) = try  element.allDescendants(where: {$0.name == \"\(element.localName)\"}).map { node in",
    ...
    private func serializer(at indentation: Indentation) -> [LineOfCode] {
        ...
        return indentation.apply(
            toFirstLine: "\(override)func serialize(_ element: AEXMLElement) throws {",
            nestedLines:
            properties.flatMap { property -> [LineOfCode] in
                let element = property.element.name
                ...
                case .identifier:
                    return [
                        "let \(property.name)Node = element.makeChildNode(nameSpaceName: \"\(element.localName)\", uri: \"\(element.uri)\")",
                ...
                case .array(.identifier):
                    return [
                        ...
                        "    let itemNode = element.makeChildNode(nameSpaceName: \"\(element.localName)\", uri: \"\(element.uri)\")",
...
extension SwiftEnum {
    public func toLinesOfCode(at indentation: Indentation) -> [LineOfCode] {
        return indentation.apply(
            toFirstLine: "enum \(name): \(rawType.toSwiftCode()),  XMLDeserializable {",
            nestedLines:      linesOfCodeForBody(at:),
            andLastLine: "}")
    }
    ...
    private func linesOfCodeForXMLDeserializer(at indentation: Indentation) -> [LineOfCode] {
        ...
        return indentation.apply(
            toFirstLine: "init(deserialize element: AEXMLElement) throws {",
            nestedLines: ["self.init(rawValue: element.value!)!"],
            andLastLine: "}")
    }
    ...
    private func linesOfCodeForXMLSerializer(at indentation: Indentation) -> [LineOfCode] {
        // TODO: no force unwraps
        return indentation.apply(
            toFirstLine: "func serialize(_ element: AEXMLElement) throws {",
            nestedLines: ["element.value = self.rawValue"],
            andLastLine: "}")
    }
...
extension ServiceMethod: LinesOfCodeConvertible {
    func toLinesOfCode(at indentation: Indentation) -> [LineOfCode] {
        return asyncCall(at: indentation)
    }

    func asyncCall(at indentation: Indentation) -> [LineOfCode] {
        let signature = (input.type.arguments + ["complete: @escaping (\(responseType())) -> Void"]).joined(separator: ", ")
        ...
        let lines = [
            ...
            "    let url = \"\(action?.absoluteString ?? "")\"",
            "    let mainName = \"ns0:\(input.element.localName)\"",
            "    let call = \(input.type.name)(\(arguments))",
            "    let soapRequest = try makeRequest(call: call, url: url, mainName: mainName)",
            "    makeCall(request: soapRequest, complete: complete)",
            "}",
        ]
